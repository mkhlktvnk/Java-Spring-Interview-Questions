...
BeanScope в Spring Framework определяет скоуп (область видимости) объекта-бина. Он определяет, как долго будет
существовать и каким образом будет доступен каждый экземпляр бина в контейнере. В Spring Framework доступны следующие
варианты BeanScope:

1. Singleton: Все запросы к контейнеру Spring возвращают один и тот же экземпляр бина. Это является значением по
   умолчанию для бинов в Spring Framework.

2. Prototype: Каждый запрос к контейнеру Spring возвращает новый экземпляр бина. Каждый экземпляр бина является
   отдельным объектом.

3. Request: Каждый HTTP-запрос, обрабатываемый веб-приложением, получает новый экземпляр бина. Действует только в
   контексте веб-приложений.

4. Session: Каждая сессия пользователя в веб-приложении получает новый экземпляр бина. Действует только в контексте
   веб-приложений.

5. GlobalSession: Такой же, как и сессионный скоуп, но применяется только в портлетах.

6. Custom: Spring Framework позволяет определить собственные пользовательские области видимости путем реализации
